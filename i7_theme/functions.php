...
function my_scripts(){
  ...
  wp_enqueue_script('main_js' , get_template_directory_uri() . '/js/custom.js', array(), '17122021', true);
	  ...
	  $arg = array(
		...
		if ($posts) :
  		...
  		$skey = str_replace("/", "\/", $skey);
			$title = preg_replace('/'.$skey.'/i', '<strong class="search_expt">' . $s . '</strong>', $title);
			// echo implode('|', $keys);
			echo '<li><a href="' . $link . '">' . strtoupper($title) . '</a></li>';
